...
![](vari_importance.png)
What we will also see is that while it is possible to predict which bakers will be winners, runner-ups, and won't win, a random forest model's ability to predict the outcome is only slightly better than a model that will always predict a baker to lose.
...
In every episode of [TGBBO](https://en.wikipedia.org/wiki/The_Great_British_Bake_Off), contestants will have to compete in three challenges: the Signature challenge, the Technical challenge, and the Showstopper challenge. For the Signature and Showstopper challenges, the contestants are allowed to make whatever they'd like as long as it stays within the theme of the challenge. For the Technical challenge, the bakers had to make a specific dish given an outline of what to make. Afterward, the bakers would be evaluated on their performance and each episode will see 1-2 bakers eliminated from the competition. This goes on until the finals where the last three bakers compete against each other to crown one of them as the winner.

Something introduced in the second season of TGBBO was the [Star Baker award](https://en.wikipedia.org/wiki/The_Great_British_Bake_Off_series_2). This award is given to contestants who had an overall great performance in the Signature, Technical, and Showstopper challenges. Every episode would usually see one baker be given the Star Baker award, but sometimes two or none of the bakers would receive it.
